...
* Two modes, normal and aggressive, make sure that's the same on both sides 
  ...
  * Agressive - works on fiber and copper 
...
* Fast UDLD - timers in the few-hundred ms range, default 200ms 
  * Fast UDLD enabled if bot hsides have fast configured and have negotiated to move from slow to fast 
* Configuration: 
...
* Also, it doesn't look like private VLANs are on here either
...
* Tagged with VLAN 0 (wtf?) - try to packet cap this
...
* Authentication 
  * `(config)# vtp password CICSO`
    ...
    * Revision number includedi n the salt, so revision number changes hash 
* VTP Pruning 
